...
loop do
  ...
  # print results
  puts RESULTS
  puts "#{MONTHLY_PAYMENT.ljust(25)}#{dollar_format(monthly_payment)}"
  puts "#{MONTHLY_RATE.ljust(25)}#{percentage_format(monthly_rate)}"
  puts "#{NUM_PAYMENTS.ljust(25)}#{term_in_months}"
  puts "#{TOTAL_PAYMENTS.ljust(25)}#{dollar_format(total_payments)}"
  puts "#{TOTAL_INTEREST.ljust(25)}#{dollar_format(total_interest)}"
